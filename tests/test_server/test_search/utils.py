...
async def enrich_runs(runs: list[Run], async_session: AsyncSession) -> Sequence[Run]:
...
async def enrich_datasets(datasets: list[Dataset], async_session: AsyncSession) -> Sequence[Dataset]:
    query = (
        select(Dataset)
...
async def enrich_jobs(jobs: list[Job], async_session: AsyncSession) -> Sequence[Job]:
    query = (
        select(Job)
