...
async def test_get_dataset_lineage_with_granularity_run(
    ...
    jobs, runs, _, datasets, inputs, _ = lineage_with_same_dataset
    jobs = await enrich_jobs(jobs, async_session)
    runs = await enrich_runs(runs, async_session)
    [dataset] = await enrich_datasets(datasets, async_session)
    inputs = [input for input in inputs if input.dataset_id == dataset.id]
    input_stats = await relation_stats_by_runs(inputs)
        ...
        + [
            {
                ...
                "to": {"kind": "RUN", "id": str(run.id)},
                "num_bytes": input_stats[(str(run.id), dataset.id)]["num_bytes"],
                "num_rows": input_stats[(str(run.id), dataset.id)]["num_rows"],
                "num_files": input_stats[(str(run.id), dataset.id)]["num_files"],
                "last_interaction_at": input_stats[(str(run.id), dataset.id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
            for run in runs
...
async def test_get_dataset_lineage_with_granularity_job(
    ...
    [dataset] = await enrich_datasets(datasets, async_session)
    inputs = [input for input in inputs if input.dataset_id == dataset.id]
    input_stats = await relation_stats_by_jobs(inputs)
        ...
        "relations": [
            {
                ...
                "num_bytes": input_stats[(str(job.id), dataset.id)]["num_bytes"],
                "num_rows": input_stats[(str(job.id), dataset.id)]["num_rows"],
                "num_files": input_stats[(str(job.id), dataset.id)]["num_files"],
                "last_interaction_at": input_stats[(str(job.id), dataset.id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
            for job in jobs
...
async def test_get_dataset_lineage_with_granularity_operation(
    ...
    jobs, runs, operations, datasets, inputs, _ = lineage_with_same_dataset

    jobs = await enrich_jobs(jobs, async_session)
    runs = await enrich_runs(runs, async_session)
    [dataset] = await enrich_datasets(datasets, async_session)
    inputs = [input for input in inputs if input.dataset_id == dataset.id]
    input_stats = await relation_stats_by_operations(inputs)
        ...
        + [
            {
                ...
                "from": {"kind": "DATASET", "id": dataset.id},
                "to": {"kind": "OPERATION", "id": str(operation.id)},
                "num_bytes": input_stats[(str(operation.id), dataset.id)]["num_bytes"],
                "num_rows": input_stats[(str(operation.id), dataset.id)]["num_rows"],
                "num_files": input_stats[(str(operation.id), dataset.id)]["num_files"],
                "last_interaction_at": input_stats[(str(operation.id), dataset.id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
...
async def test_get_dataset_lineage_with_direction_both(
    ...
    jobs, runs, _, datasets, inputs, outputs = lineage_with_same_dataset
    jobs = await enrich_jobs(jobs, async_session)
    runs = await enrich_runs(runs, async_session)
    [dataset] = await enrich_datasets(datasets, async_session)
    inputs = [input for input in inputs if input.dataset_id == dataset.id]
    input_stats = await relation_stats_by_runs(inputs)
    outputs = [output for output in outputs if output.dataset_id == dataset.id]
    output_stats = await relation_stats_by_runs(outputs)
        ...
        + [
            {
                ...
                "type": "APPEND",
                "num_bytes": output_stats[(str(run.id), dataset.id)]["num_bytes"],
                "num_rows": output_stats[(str(run.id), dataset.id)]["num_rows"],
                "num_files": output_stats[(str(run.id), dataset.id)]["num_files"],
                "last_interaction_at": output_stats[(str(run.id), dataset.id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
            for run in runs
...
async def test_get_dataset_lineage_with_direction_and_until_and_granularity_operation(
    ...
    jobs, runs, all_operations, datasets, _, outputs = lineage_with_same_dataset
    jobs = await enrich_jobs(jobs, async_session)
    runs = await enrich_runs(runs, async_session)
    [dataset] = await enrich_datasets(datasets, async_session)
    outputs = [output for output in outputs if output.dataset_id == dataset.id]
    output_stats = await relation_stats_by_operations(outputs)
        ...
        + [
            {
                ...
                "to": {"kind": "DATASET", "id": dataset.id},
                "type": "APPEND",
                "num_bytes": output_stats[(str(operation.id), dataset.id)]["num_bytes"],
                "num_rows": output_stats[(str(operation.id), dataset.id)]["num_rows"],
                "num_files": output_stats[(str(operation.id), dataset.id)]["num_files"],
                "last_interaction_at": output_stats[(str(operation.id), dataset.id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
            for operation in operations
        ...
        + [
            {
                ...
                "num_bytes": input_stats[(str(input.run_id), input.dataset_id)]["num_bytes"],
                "num_rows": input_stats[(str(input.run_id), input.dataset_id)]["num_rows"],
                "num_files": input_stats[(str(input.run_id), input.dataset_id)]["num_files"],
                "last_interaction_at": input_stats[(str(input.run_id), input.dataset_id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
            for input in inputs
        ...
        + [
            {
                ...
                "type": "APPEND",
                "num_bytes": output_stats[(str(output.run_id), output.dataset_id)]["num_bytes"],
                "num_rows": output_stats[(str(output.run_id), output.dataset_id)]["num_rows"],
                "num_files": output_stats[(str(output.run_id), output.dataset_id)]["num_files"],
                "last_interaction_at": output_stats[(str(output.run_id), output.dataset_id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
...
async def test_get_dataset_lineage_with_depth_and_granularity_job(
    ...
    outputs = second_level_outputs
    output_stats = await relation_stats_by_jobs(outputs)
    inputs = first_level_inputs + third_level_inputs
    input_stats = await relation_stats_by_jobs(inputs)
        ...
        "relations": [
            {
                ...
                "num_bytes": input_stats[(str(input.job_id), input.dataset_id)]["num_bytes"],
                "num_rows": input_stats[(str(input.job_id), input.dataset_id)]["num_rows"],
                "num_files": input_stats[(str(input.job_id), input.dataset_id)]["num_files"],
                "last_interaction_at": input_stats[(str(input.job_id), input.dataset_id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
            for input in inputs
        ...
        + [
            {
                ...
                "to": {"kind": "DATASET", "id": output.dataset_id},
                "type": "APPEND",
                "num_bytes": output_stats[(str(output.job_id), output.dataset_id)]["num_bytes"],
                "num_rows": output_stats[(str(output.job_id), output.dataset_id)]["num_rows"],
                "num_files": output_stats[(str(output.job_id), output.dataset_id)]["num_files"],
                "last_interaction_at": output_stats[(str(output.job_id), output.dataset_id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
            for output in outputs
        ...
        + [
            {
                ...
                "num_bytes": input_stats[(str(input.operation_id), input.dataset_id)]["num_bytes"],
                "num_rows": input_stats[(str(input.operation_id), input.dataset_id)]["num_rows"],
                "num_files": input_stats[(str(input.operation_id), input.dataset_id)]["num_files"],
                "last_interaction_at": input_stats[(str(input.operation_id), input.dataset_id)]["created_at"].strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ",
                ),
            }
        ...
        + [
            {
                ...
                "to": {"kind": "DATASET", "id": output.dataset_id},
                "type": "APPEND",
                "num_bytes": output_stats[(str(output.operation_id), output.dataset_id)]["num_bytes"],
                "num_rows": output_stats[(str(output.operation_id), output.dataset_id)]["num_rows"],
                "num_files": output_stats[(str(output.operation_id), output.dataset_id)]["num_files"],
                "last_interaction_at": output_stats[(str(output.operation_id), output.dataset_id)][
                    "created_at"
...
async def test_get_dataset_lineage_with_depth_and_granularity_run_ignore_cycles(
    ...
    jobs, runs, _, [dataset], inputs, outputs = lineage_with_same_dataset
...
async def test_get_dataset_lineage_with_depth_and_granularity_operation_ignore_cycles(
    ...
    jobs, runs, operations, [dataset], inputs, outputs = lineage_with_same_dataset
